...
@section('content')
                ...
                <tbody>
                    @foreach ($empresas as $empresa)
                        <tr>
                            ...
                            <td>{{ $empresa->EMP }}</td>
                            <td>{{ ( $empresa->añoEmpresa ) ? formatDate($empresa->añoEmpresa) : '' }}</td>
                            <td>{{ ( $empresa->tipo ) ? $empresa->tipo->nameTipo : 'Sin tipo' }}</td>
                            <td>{{ $empresa->observacionesEmpresa }}</td>
                            <td>
                                @component('components.actions-button')
                                    <button 
                                        data-id="{{ $empresa->idEmpresa }}"
                                        data-ano="{{ $empresa->añoEmpresa }}"
                                        data-emp="{{ $empresa->EMP }}"
                                        data-tipo="{{ ( $empresa->tipo ) ? $empresa->tipo->nameTipo : 'Sin tipo' }}"
                                        data-observaciones="{{ $empresa->observacionesEmpresa }}"
    ...
    @component('components.modal-component', [
        ...
        <form id="formCreateEmpresa" action="{{ route('admin.empresas.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="form-group col-md-6">
                    ...
                    <select id="createTipoEmpresa" name="tipoEmpresa" class="form-control">
                        ...
                        @foreach ($tipos as $tipoEmpresa)
                            <option value="{{ $tipoEmpresa->idtiposEmpresa }}">{{ $tipoEmpresa->nameTipo }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    @component('components.modal-component', [
        ...
        <form id="formEditEmpresa" action="{{ route('admin.empresas.update', 0) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
