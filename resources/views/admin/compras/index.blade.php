...
@section('content')
                ...
                <tbody>
                    @foreach ($compras as $compra)
                        ...
                        >
                            ...
                            <td>{{ $compra->NFacturaCompra }}</td>
                            <td>{{ $compra->proveedor->nombreProveedor }}</td>
                            <td>{{ $compra->empresa->EMP }}</td>
                            <td>{{ ($compra->formaPago == 1) ? 'Banco' : 'Efectivo' }}</td>
                            <td>{{ number_format($compra->totalFactura, 2, ',', '.') }} €</td>
                            <td>{{ ($compra->totalExacto) ? number_format($compra->totalExacto, 2, ',', '.') : '0.00' }} €</td>
                            <td
                                data-fulltext="{{ $compra->ObservacionesCompras }}"
                                data-bs-toggle="tooltip" 
                                data-bs-placement="top"
                                title="{{ $compra->ObservacionesCompras }}"
                                class="text-truncate openqQuickEdit"
                                data-fieldName="ObservacionesCompras"
                                data-type="text"
                                data-parteid="{{ $compra->idCompra }}"
                            >
                                @if ($compra->ObservacionesCompras)
                                    
                                    {{ Str::limit($compra->ObservacionesCompras, 10) }}
                                @endif
                            </td>
                            <td
                                data-fulltext="{{ $compra->notas1 }}"
                                data-bs-toggle="tooltip" 
                                data-bs-placement="top"
                                title="{{ $compra->notas1 }}"
                                class="text-truncate openqQuickEdit"
                                ...
                                >
                                @if ($compra->notas1)
                                    {{ Str::limit($compra->notas1, 10) }}
                                @endif
                            </td>
                            <td
                                data-fulltext="{{ $compra->notas2 }}"
                                data-bs-toggle="tooltip" 
                                data-bs-placement="top"
                                title="{{ $compra->notas2 }}"
                                class="text-truncate openqQuickEdit"
                            ...
                            <td>
                                @component('components.actions-button')
                                    ...
                                    <button
                                        ...
                                        data-lineas="{{ $compra->lineas }}"
                                        data-archivo="{{ $compra->archivos }}"
                                        data-proximasfecha="{{ $compra->plazos }}"
                                    >
                                        <div class="d-flex justify-between align-items-center align-content-center flex-wrap flex-column">
    ...
    @component('components.modal-component', [
        ...
        <div id="accordion">
            ...
            {{-- Acordeon de detalles de la compra --}}
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#btnCreateProveedor').on('click', function(event){
                ...
                $.ajax({
                    url: '{{ route("admin.proveedores.storeApi") }}',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            const calcularTotales = (id) => {
                return new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => {
                        if (id) {
                            $.ajax({
                                url: `{{ route('admin.compras.updatesum', ':id') }}`.replace(':id', id),
                                method: 'POST',
                                data: {
                                    _token: '{{ csrf_token() }}',
                        ...
                        if (compraGuardadaGlobal) {
                            ...
                            $('#modalCreateCompra #addNewLine').click(function() {
                                ...
                                $(`#modalCreateCompra #cod_prov${globalLineas}`).off('change').on('change', function() {
                                    ...
                                    $.ajax({
                                        url: `{{ route('admin.compras.getArticuloByCodigo') }}`,
                                        method: 'GET',
                                        data: {
                                            cod_prov: cod_prov
                                        },
            ...
            $('#btnSaveEditLineaCreate').click(function () {
                ...
                $.ajax({
                    url: `{{ route('admin.lineas.update', ':id') }}`.replace(':id', id),
                    method: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        descripcion,
            ...
            $('#modalCreateCompra #elementsToShow').on('click', '.deleteLinea', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('admin.lineas.destroy', ':id') }}`.replace(':id', id),
                            method: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            table.on('click', '.modalDetailsCompras', function () {
                ...
                if (archivo) {
                    ...
                    let pdfViewer = `
                        <embed src="{{ asset("archivos/compras/") }}/${ archivoFinal }" type="application/pdf" width="100%" height="600px">
                    `;
            ...
            $('#modalEditLinea #btnSaveEditLinea').click(function() {
                ...
                $.ajax({
                    url: `{{ route('admin.lineas.update', ':id') }}`.replace(':id', lineaId),
                    method: 'POST',
                    data: formData,
                    success: function(response) {
            ...
            $('#elementsToShowEdit').on('click', '.deleteLineaCompra', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('admin.lineas.destroy', ':id') }}`.replace(':id', lineaId),
                            method: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            $('#newLinesContainerEdit').on('click', '.saveLineaEdit', function (event) {
                ...
                if ( proveedor.cifProveedor == undefined || proveedor.cifProveedor == null || proveedor.cifProveedor == 'undefined' || !proveedor.cifProveedor ) {
                    $.ajax({
                        'url': '{{ route("admin.lineas.getInfoToStore") }}',
                        'method': 'POST',
                        'data': {
                        _token: '{{ csrf_token() }}',
                        proveedor_id: proveedor.idProveedor,
            ...
            table.on('click', '.deleteCompra', function(event){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('admin.compras.destroy', ':id') }}`.replace(':id', compraId),
                            method: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            $('#modalCreateCompra').on('hidden.bs.modal', function (e) {
                ...
                if (elements.find('tr').length > 0) {
                    ...
                    $.ajax({
                        url: '{{ route("admin.compras.sendTelegram") }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id
