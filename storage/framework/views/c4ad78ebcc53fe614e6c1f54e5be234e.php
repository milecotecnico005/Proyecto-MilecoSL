...
<?php $__env->startSection('body'); ?>
    ...
    <section id="sidebar" class="hide">
        <a href="#" class="brand">
            ...
            <span class="text">MILECO <strong>SL</strong></span>
        </a>
        <ul style="margin-top: -50px" class="side-menu top"
    ...
    <section class="wrapper" id="content">
        ...
        <div id="toastContainer" style="position: fixed; bottom: 50px; right: 50px; z-index: 9999999;"></div>
        ...
        <nav>
			...
			<a href="#" class="nav-linkDashboard">Módulos</a>
			<form action="#">
				<div class="form-input">
					<input class="searchModulesFilter" type="search" placeholder="Search...">
        ...
        <script>
            ...
            $(document).ready(function () {
                $('#clearCache').on('click', function () {
                    // Forzar recarga sin caché añadiendo un parámetro único a la URL
                    const currentUrl = window.location.href.split('?')[0]; // Elimina parámetros existentes
                    window.location.href = `${currentUrl}?cache_buster=${new Date().getTime()}`;
                });
                ...
                switchMode.addEventListener('change', () => {
                    if (switchMode.checked) {
                        root.style.setProperty('--grey', '#333');
                        root.style.setProperty('--blue', '#f4f4f4');
                        root.style.setProperty('--white', '#fff');
                    } else {
                        root.style.setProperty('--grey', '#f4f4f4');
                        root.style.setProperty('--blue', '#007bff');
                        root.style.setProperty('--white', '#000');
                    }
                });

                // Limpiar caché de la aplicación
                $('#clearCacheApp').on('click', function () {
                    $.ajax({
                        url: "<?php echo e(route('clearCacheApp')); ?>",
                        type: 'POST',
                        data: {
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                text: 'La caché de la aplicación ha sido limpiada con éxito.',
                        ...
                        error: function(error) {
                            ...
                            Swal.fire({
                                ...
                                text: 'Ha ocurrido un error al intentar limpiar la caché de la aplicación.',
                                showConfirmButton: false,
                                timer: 2000
                            });
                        }
            ...
            // Filtrar elementos del sidebar por el texto del input y redirigir a la url
            const filter = document.querySelector('.searchModulesFilter');
            const links = document.querySelectorAll('.side-menu a');

            const logoutButton = document.querySelector('.logoutButton');
            const searchBtn = document.querySelector('.search-btn');
        
            // Crear desplegable suggestions con los elementos filtrados
            filter.addEventListener('input', (e) => {
                // Hacer foco en el input
                filter.focus();

                const value = e.target.value.trim().toLowerCase(); // Captura el valor del input y lo convierte a minúsculas
                const suggestions = document.createElement('div');
                suggestions.classList.add('suggestions', 'suggestionsFilterContainer');
                suggestions.style.position = 'absolute';
                suggestions.style.top = '100%';
                suggestions.style.width = `${filter.offsetWidth}px`; // Ancho del input
                suggestions.style.zIndex = '9999999';
                suggestions.style.border = '1px solid #ccc';
                suggestions.style.borderTop = 'none';
                suggestions.style.borderRadius = '0 0 5px 5px';
                suggestions.style.display = 'none';
                suggestions.style.maxHeight = '200px';
                suggestions.style.overflowY = 'auto';
                suggestions.style.boxShadow = '0 0 5px rgba(0, 0, 0, 0.1)';
                suggestions.style.padding = '5px 0';
                suggestions.style.boxSizing = 'border-box';

                // Limpiar las sugerencias anteriores antes de agregar nuevas
                const existingSuggestions = document.querySelector('.suggestions');
                if (existingSuggestions) {
                    existingSuggestions.remove();
                }

                // Crear un conjunto para almacenar los textos únicos
                const uniqueSuggestions = new Set();

                // Recorrer los links y buscar coincidencias parciales
                links.forEach(link => {

                    // si el link tiene el attributo data-toggle="collapse" no se incluye en las sugerencias
                    ...
                    const linkHref = link.href; // Obtener el href correcto del link


                    // Excluir enlaces con href="#"
                    if (value && linkText.includes(value) && linkHref && linkHref !== 'configApp#') {
                        // Verificar si el texto ya fue agregado
                        if (!uniqueSuggestions.has(linkText)) {
                            uniqueSuggestions.add(linkText); // Agregar al conjunto

                            const suggestion = document.createElement('a');
                            suggestion.href = linkHref; // Asegurarse de asignar el href correcto
                            suggestion.textContent = link.textContent;
                            suggestion.style.display = 'block';
                            suggestion.style.padding = '5px 10px';
                            suggestion.style.textDecoration = 'none';
                            suggestion.style.transition = 'background-color 0.3s';
                ...
                // Mostrar el contenedor de sugerencias solo si hay sugerencias
                if (suggestions.children.length > 0) {
                    suggestions.style.display = 'block';
                    filter.parentElement.appendChild(suggestions);
                }
            });
            ...
            logoutButton.addEventListener('click',function(event) {
                ...
                // redireccionar con ajax para cerrar sesión
                $.ajax({
                    url: '/logout',
                    type: 'POST',
                    data: {
...
<?php echo $__env->make('adminlte::master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Clases_Programacion\Clientes\MILECOSL\milecosl\vendor\jeroennoten\laravel-adminlte\src/../resources/views/page.blade.php ENDPATH**/ ?>